...
// FromBytes builds Strings structure from a sequence of bytes. The input data
// does not include option code and length bytes.
func (s *Strings) FromBytes(data []byte) error {
	buf := uio.NewBigEndianBuffer(data)
	for buf.Has(2) {
		length := buf.Read16()
		*s = append(*s, string(buf.CopyN(int(length))))
	}
	return buf.FinError()
}
