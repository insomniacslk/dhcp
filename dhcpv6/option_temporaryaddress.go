...
func (op *OptIATA) String() string {
	return fmt.Sprintf("IATA: {IAID=%#x Options=%v}",
		op.IaId, op.Options)
}

func (op *OptIATA) LongString(indent int) string {
	return fmt.Sprintf("IATA: {IAID=%#x Options=%s}",
		op.IaId, op.Options.LongString(indent))
}
